...
export default function fileSearchTools(pi: ExtensionAPI) {
  ...
  pi.registerTool<ReturnType<typeof fdParameters>, FdToolDetails>({
    ...
    renderResult(result, { expanded, isPartial }, theme) {
      if (isPartial) return new Text(theme.fg("warning", "Searching..."), 1, 0);
      const details = result.details;
      if (!details || !details.matchCount) {
        return new Text(theme.fg("dim", "No files found"), 1, 0);
      }
      let text = theme.fg(
        "success",
  ...
  pi.registerTool<ReturnType<typeof rgParameters>, RgToolDetails>({
    ...
    renderResult(result, { expanded, isPartial }, theme) {
      ...
      if (!details || !details.outputLines) {
        return new Text(theme.fg("dim", "No matches found"), 1, 0);
      }
      let text = theme.fg(
        "success",
