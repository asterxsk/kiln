...
function renderThinking(
  ...
  // Preserve paragraph breaks so multi-paragraph reasoning doesn't collapse
  // into one wall of text; each paragraph gets its own `~` continuation.
  ...
  for (let i = 0; i < wrapped.length; i++) {
    ...
    out.push(
      truncateToWidth(
        (i === 0 ? prefix : "  ") + theme.fg("muted", theme.italic(line)),
        width,
      ),
    );
  }
...
function renderAssistantItem(
  ...
  // A rendered reasoning block is followed by a blank line so the thinking
  // doesn't run into the answer/tool call that comes next.
...
export function buildTranscriptLines(
  ...
  if (snap.liveAssistant) {
    ...
    if (text.trim()) {
      // Separate the streamed answer from the reasoning block above it.
      if (thinking.trim()) out.push("");
